...
func main() {
	...
	if err != nil {
		log.Fatalf("migration database connection failed: %v", err)
	}
	defer db.Close(ctx)
	...
	if _, err := db.Exec(ctx, "SELECT pg_advisory_lock(682441002)"); err != nil {
		log.Fatal(err)
	}
	...
	for _, path := range files {
		...
		if match == nil {
			log.Fatalf("invalid migration filename: %s", path)
		}
		...
		if strings.TrimSpace(string(sql)) == "" {
			log.Fatalf("empty migration: %s", path)
		}
		tx, err := db.Begin(ctx)
		if err != nil {
			log.Fatal(err)
		...
		if err != nil {
			log.Fatalf("migration %d failed: %v", version, err)
		}
