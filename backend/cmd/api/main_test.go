...
func TestCaptchaCreateVerifyIsOneTime(t *testing.T) {
	...
	if err := json.Unmarshal(verified.Body.Bytes(), &ticket); err != nil || ticket.CaptchaTicket == "" {
		t.Fatal("missing ticket")
	}
	if _, ok := s.store.challenges[c.ID]; ok {
		t.Fatal("challenge was not consumed")
	}
	if !s.consumeTicket(ticket.CaptchaTicket, payload.AnonymousSessionToken, "admin_login", "") {
		t.Fatal("ticket must be consumable once")
	}
	if s.consumeTicket(ticket.CaptchaTicket, payload.AnonymousSessionToken, "admin_login", "") {
...
func TestCaptchaRejectsWrongAnswerAndExpiresAfterAttempts(t *testing.T) {
	s := testServer()
	c := &captchaChallenge{ID: "c1", Session: "s1", Purpose: "login", Expires: nowPlus(60)}
	c.AnswerHash = s.answerHash(c.ID, "ABCDE")
	s.store.challenges[c.ID] = c
	s.store.sessions[c.Session] = nowPlus(60)
	for i := 0; i < 3; i++ {
		body, _ := json.Marshal(verifyRequest{ChallengeID: c.ID, Answer: "WRONG", Purpose: c.Purpose, AnonymousSessionToken: c.Session})
		r := httptest.NewRecorder()
		s.verifyChallenge(r, httptest.NewRequest(http.MethodPost, "/verify", bytes.NewReader(body)))
...
func nowPlus(seconds int64) (result time.Time) {
