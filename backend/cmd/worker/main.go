...
func main() {
	...
	if err != nil || interval < time.Second {
		log.Fatal("WORKER_POLL_INTERVAL must be at least one second")
	}
	...
	for {
		select {
		case <-ticker.C:
			if err := publishOutbox(ctx, db); err != nil {
				log.Printf("outbox poll failed: %v", err)
			}
		case <-stop:
			log.Println("hhy worker stopped")
			return
		}
	}
}
...
func publishOutbox(ctx context.Context, db *pgx.Conn) error {
	...
	for rows.Next() {
		...
		ids = append(ids, id)
		log.Printf("outbox event accepted: %s (%s)", id, eventType)
	}
	if err := rows.Err(); err != nil {
		return err
	}
	if len(ids) > 0 {
		if _, err := tx.Exec(ctx, "UPDATE outbox_events SET published_at = now() WHERE id = ANY($1::uuid[])", ids); err != nil {
			return err
		}
	}
